...
@implementation MHShapeNode

-(instancetype)copyWithZone:(NSZone *)zone {
    MHShapeNode* shape = [[self class] node];
...
+(instancetype)squareWithWidth:(CGFloat)width andColor:(SKColor*)color{
...
+(instancetype)rectangleWithSize:(CGSize)size andColor:(SKColor*)color{
...
+(instancetype)rectangleRoundedWithSize:(CGSize)size andCornerRadius:(CGFloat)radius andColor:(SKColor*)color{
    CGRect rect = CGRectMake(-size.width / 2.0, -size.height / 2.0, size.width, size.height);
...
+(instancetype)shapeWithPath:(CGPathRef)path andColor:(SKColor *)color {
    MHShapeNode* shapeNode = [[self class] node];
    shapeNode.fillColor = color;
    shapeNode.path = path;
    return shapeNode;
...
-(instancetype)init {
    ...
    if (self = [super init]) {
        ...
        self.name = @"MHShapeNode";
        
        null = [SKNode node];
        null.name = @"MHShapeNodeNULL";
        [self addChild:null];
        
        drawSprite = [SKSpriteNode node];
        drawSprite.name = @"MHShapeNodeDrawSprite";
...
//    SKULog(0,@"bounding: %f %f %f %f", enclosure.origin.x, enclosure.origin.y, enclosure.size.width, enclosure.size.height);
    ...
    // DR modifications 7/12/21: drawing the texture in higher resolution for improved rendering quality (at the cost of a much greater memory consumption - need to work on fixing that)
    static CGFloat textureScalingFactor = 2;        // by what factor shall we scale up the image size? the memory increase factor is the square of this number...
    
    CGSize actualBoundingSize;
    actualBoundingSize.width = textureScalingFactor * boundingSize.width;
    actualBoundingSize.height = textureScalingFactor * boundingSize.height;

    CGSize imageSize = actualBoundingSize; // boundingSize;
    ...
    // DR modifications 7/12/21: the code line below was added
    CGContextScaleCTM(newContext, textureScalingFactor, textureScalingFactor);
